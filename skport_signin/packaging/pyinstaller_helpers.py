...
def build_pyinstaller(mode: str, project_root: Path) -> Path:
    ...
    spec_path = project_root / "packaging" / "skport_signin.spec"
    ...
    env = os.environ.copy()
    env["SKPORT_SIGNIN_PYINSTALLER_MODE"] = mode
    command = [
        sys.executable,
        "-m",
        "PyInstaller",
...
def create_release_tree(mode: str, project_root: Path) -> Path:
    ...
    release_root = releases_dir / f"Skport_Signin-Windows-{mode}"
    ...
    if mode == "onedir":
        ...
        shutil.copy2(build_output / "skport_signin.exe", release_root / "skport_signin.exe")
