...
class _FakeResponse:
    def __init__(self, status: int, payload: dict | None = None, ok: bool = True) -> None:
        self.status = status
        self._payload = payload or {}
        self.ok = ok
        self.url = "https://zonai.skport.com/web/v1/game/endfield/attendance"
...
class _FakePage:
    ...
    def expect_response(self, predicate, timeout: int):
        return _FakeExpectResponse(self._responses.pop(0))
...
class SignInTests(unittest.TestCase):
    def test_run_browser_sign_in_returns_message_then_status_on_success(self) -> None:
        ...
        fake_page = _FakePage(
            [
                _FakeResponse(200, attendance_payload),
                _FakeResponse(200, {}, ok=True),
                _FakeResponse(200, refreshed_payload),
            ]
        )
